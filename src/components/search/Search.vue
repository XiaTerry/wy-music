<template>
  <div id="serach">
    ...
    <div class="search-text">
      <span @click="addQuery(item.first)" v-for="(item,index) in name" :key="index">{{item.first}}</span>
    </div>
    <div class="search-history">
      <p class="search-hot">历史记录</p>
      <div class="search-text">
        <span @click="addQuery(item)" v-for="(item,index) in history" :key="index">{{item}}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    _getSearchResult(keyword) {
      ...
      getSearchResult((this.query = ' ' ? keywords : this.query), 30).then(
        (res) => {
    ...
    ...mapMutations(['SET_CURRENT_SONG_ID', 'SET_PLAYER_VISIBLE']),

    ...mapActions(['setCurrentSongId', 'chagePlayerVisible']),
  },
};
</script>
